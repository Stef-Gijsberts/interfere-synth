...
impl AudioGenerator {
    pub fn audio_requested(&mut self, buffer: &mut [(f64, f64)], samplerate_in_hz: f64, dglobal: &DGlobalRow, dvoices: &DVoicesMatrix) {
        ...
        for (l, r) in buffer {
            ...
            if !audio_available {
                self.oscillator.voices_audio_requested(&mut self.voicebuffer, samplerate_in_hz, dvoices);
                self.filter.voices_audio_requested(&mut self.voicebuffer, samplerate_in_hz, dvoices);

                self.idx_head_voicebuffer = 0;
            }

            *l = self.voicebuffer[self.idx_head_voicebuffer].iter().sum::<f64>() / 16.0; // TODO: mix per voice?
            *r = *l;
